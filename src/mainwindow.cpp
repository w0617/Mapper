...
{
    if( StereoImage::depthImageShow )
    {
//        if( this->m_stereo->computeDisparityMode == BM_MODE )
//        {
//            this->m_3dReconstructionThread->bmDepthImage = this->m_3dReconstructionThread->bm.updateBM();
//        }

//        if( this->m_stereo->computeDisparityMode == SGBM_MODE )
//        {
//            this->m_3dReconstructionThread->bmDepthImage = this->m_3dReconstructionThread->sgbm.updateSGBM();
//        }
...
void MainWindow::on_DisparityModeBox_activated(const QString &arg1)
{
    if( arg1 == "     BM-Mode" )
    {
        this->m_stereo->computeDisparityMode = BM_MODE;
        cout<<"BM_MODE to compute disparity."<<endl;
        this->slot_reset();
    }
    if( arg1 == "    SGBM-Mode" )
    {
        this->m_stereo->computeDisparityMode = SGBM_MODE;
        cout<<"SGBM_MODE to compute disparity."<<endl;
        this->slot_reset();
    }
    if( arg1 == "     Elas-Mode" )
    {
        this->m_stereo->computeDisparityMode = ELAS_MODE;
...
{
    Mat leftImage(StereoImage::m_forSaveDepth->height, StereoImage::m_forSaveDepth->step, CV_8U);
    Mat rightImage(StereoImage::m_forSaveDepth->height, StereoImage::m_forSaveDepth->step, CV_8U);

    if( !m_readingLocalFileFlag )
    {
        this->m_3dReconstructionThread->bm_arrayToMat(StereoImage::m_forSaveDepth->I1, leftImage);
        this->m_3dReconstructionThread->bm_arrayToMat(StereoImage::m_forSaveDepth->I2, rightImage);

        if( this->m_stereo->computeDisparityMode == BM_MODE )
        {
            this->m_3dReconstructionThread->bm.setImages(leftImage, rightImage);
            this->m_3dReconstructionThread->bm_matToArray(m_3dReconstructionThread->bm.updateBM(), StereoImage::m_forSaveDepth->D1);
        }

        if( this->m_stereo->computeDisparityMode == SGBM_MODE )
        {
            this->m_3dReconstructionThread->sgbm.setImages(leftImage, rightImage);
            this->m_3dReconstructionThread->bm_matToArray(m_3dReconstructionThread->sgbm.updateSGBM(), StereoImage::m_forSaveDepth->D1);
        }
    }
...
void MainWindow::on_tabWidget_currentChanged (int index)
{
    if( index == 1 )
        ...
        StereoImage::depthImageShow = false;
}

void MainWindow::ShowDepthImage()
{
    cv::Mat depthImage(StereoImage::m_forSaveDepth->height, StereoImage::m_forSaveDepth->width, CV_8U);
    float *address;
    float temp;
    address = StereoImage::m_forSaveDepth->D1;
